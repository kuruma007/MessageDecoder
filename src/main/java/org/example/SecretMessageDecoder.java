    ...
    public static void decodeMessage(String urlString) {
        ...
        try {
            ...
            for (Element row : rows) {
                ...
                try {
                    int x = (cols.get(0).text().trim().matches("\\d+") ? (cols.get(0).text().trim().matches("\\d+") ? (cols.get(0).text().trim().matches("\\d+") ? Integer.parseInt(cols.get(0).text().trim()) : 0) : 0) : 0);
                    char ch = cols.get(1).text().trim().charAt(0);
                    int y = (cols.get(2).text().trim().matches("\\d+") ? (cols.get(2).text().trim().matches("\\d+") ? (cols.get(2).text().trim().matches("\\d+") ? Integer.parseInt(cols.get(2).text().trim()) : 0) : 0) : 0);
